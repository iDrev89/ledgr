...
const getTypeColor = (type: AccountTransactionType) => {
  switch (type) {
    case AccountTransactionType.INCOME:
    case AccountTransactionType.TRANSFER_IN:
      return "text-green-600 dark:text-green-400";
    case AccountTransactionType.EXPENSE:
    case AccountTransactionType.TRANSFER_OUT:
...
export const createTransactionColumns = ({
  t,
  locale = "es",
}: CreateTransactionColumnsProps): ColumnDef<AccountTransactionWithRelations>[] => [
  {
  ...
  },
  {
    ...
    header: t("type"),
    cell: ({ row }) => {
      const type = row.getValue("type") as AccountTransactionType;
      const typeMap = {
        [AccountTransactionType.INCOME]: t("typeIncome"),
        [AccountTransactionType.EXPENSE]: t("typeExpense"),
        [AccountTransactionType.TRANSFER_IN]: t("typeTransferIn"),
        [AccountTransactionType.TRANSFER_OUT]: t("typeTransferOut"),
        [AccountTransactionType.ADJUSTMENT]: t("typeAdjustment"),
      };
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const relatedAccount = row.original.relatedAccount;
      ...
      if (relatedAccount) {
        return (
          <div className="text-sm">
            <span className="font-medium">{relatedAccount.name}</span>
          </div>
        );
      }
