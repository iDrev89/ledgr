...
interface CreateCategoryColumnsProps {
  ...
  onDelete: (category: ExpenseCategoryWithRelations) => void;
  t: (key: string) => string;
}
...
export const createCategoryColumns = ({
  ...
  onDelete,
  t,
}: CreateCategoryColumnsProps): ColumnDef<ExpenseCategoryWithRelations>[] => [
  {
    accessorKey: "name",
    header: t("name"),
    cell: ({ row }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const total = (count?.expenses || 0) + (count?.expenseItems || 0);
      return (
